...
import { getUrinarySymptomsByGender } from '../config/symptoms';

interface DiagnosisStep2UrinaryProps {
  specialty: Specialty;
  gender: Gender; // 🆕 Recibir género para filtrar síntomas
  onBack: () => void;
  onContinue: (urinarySymptoms: string[]) => void;
  initialSymptoms: string[];
}

export function DiagnosisStep2Urinary({ specialty, gender, onBack, onContinue, initialSymptoms }: DiagnosisStep2UrinaryProps) {
  ...
  const urinarySymptoms = getUrinarySymptomsByGender(gender);
  ...
  return (
    <DiagnosisScreenLayout
      ...
      subtitle="Síntomas urinarios"
      stepInfo="Paso 3 de 8"
      onBack={onBack}
      footer={
        <GradientButton
          gradient={theme.gradient}
      ...
      <div className="p-6 pb-8">
        <div className={`${theme.lightBg} rounded-2xl p-4 mb-6`}>
          <p className="text-sm text-gray-700">
            Selecciona los síntomas urinarios que hayas experimentado.
            Esta información es importante para tu evaluación.
          </p>
        </div>

        <SymptomSelector
          symptoms={urinarySymptoms}
